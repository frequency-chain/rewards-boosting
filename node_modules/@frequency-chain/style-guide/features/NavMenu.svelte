<script lang="ts">// Note: Shows the buttons still in md view, but not the non-buttons
import { onMount, tick } from 'svelte';
import Button from '../atoms/Button.svelte';
import { cn } from '../utils/utils.js';
let { intent = 'dark', menuItems = [], highlightMarginTop = '0px' } = $props();
let viewportTrackingItems = $state(menuItems.filter((item) => item.viewportHighlightId).map((item) => [item.viewportHighlightId, false]));
...
onMount(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach((e) => {
            ...
            if (index > -1)
                tick().then(() => (viewportTrackingItems[index][1] = e.isIntersecting));
        });
